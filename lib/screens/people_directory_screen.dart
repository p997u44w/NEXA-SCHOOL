...
class _PeopleDirectoryScreenState extends State<PeopleDirectoryScreen> {
                            ...
                            if (isAdmin && adminSchools.isNotEmpty)
                              DropdownButtonFormField<int>(
                                ...
                                decoration: const InputDecoration(
                                    labelText: 'انتخاب مدرسه'),
                                items: adminSchools
                                    .map<DropdownMenuItem<int>>(
                                      (x) => DropdownMenuItem(
                                        value: int.tryParse('${x['id']}'),
                                        child: Text(x['name'] ?? ''),
                                      ),
                                    )
                                    .toList(),
  ...
  List<Widget> _buildDirectory() {
    if (schools.isEmpty ||
        schools.first is! Map ||
        schools.first['school'] == null) {
      return [
        const Card(
          child: Padding(
    ...
    for (final block in schools) {
      ...
      result.add(Card(
        child: ListTile(
          ...
          subtitle: Text(
              'معلم: ${teachers.length}  •  دانش‌آموز: ${students.length}'),
        ),
      ));
      ...
      for (final s in students) {
        result.add(Card(
          child: ExpansionTile(
            ...
            subtitle: Text(
                'کلاس: ${s['class_name'] ?? '-'}  •  کد ملی: ${s['national_code'] ?? '-'}'),
            children: [
              Padding(
                padding:
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('تاریخچه تکالیف',
                        style: TextStyle(fontWeight: FontWeight.bold)),
                    ...(s['assignment_history'] as List? ?? []).map(
                      (a) => ListTile(
                        dense: true,
                        title: Text(a['title'] ?? ''),
                        subtitle: Text(
                            'ثبت: ${a['submitted_at'] ?? 'ارسال نشده'} • نمره: ${a['score'] ?? '-'} / ${a['max_score'] ?? '-'}${a['is_late'] == 1 ? ' • دیرکرد' : ''}'),
                      ),
                    ),
                    const Divider(),
                    const Text('تاریخچه امتحان‌ها',
                        style: TextStyle(fontWeight: FontWeight.bold)),
                    ...(s['exam_history'] as List? ?? []).map(
                      (e) => ListTile(
                        dense: true,
                        title: Text(e['title'] ?? ''),
                        subtitle: Text(
                            'نمره: ${e['score'] ?? '-'} • ${e['submitted_at'] ?? 'شرکت نکرده'}'),
                      ),
                    ),
                  ],
                ),
